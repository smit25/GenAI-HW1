...
class GroupedQueryAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        k_repeated = torch.repeat_interleave(k, self.g, dim=1)
        v_repeated = torch.repeat_interleave(v, self.g, dim=1)
        torch.cuda.empty_cache()
        start_memory = torch.cuda.memory_allocated()
        scale = math.sqrt(k.size(-1))
        att = einsum(q, k_repeated, 'b h q d, b h k d -> b h q k') / scale
        att = att.masked_fill(self.bias[:,:,:t,:t] == 0, float('-inf'))
        att = F.softmax(att, dim=-1)
        att = self.attn_dropout(att)
        y = einsum(att, v_repeated, 'b h q t, b h t d -> b h q d')
        end_memory = torch.cuda.memory_allocated()
        y = rearrange(y, 'b h q d -> b q (h d)') # re-assemble all head outputs side by side
